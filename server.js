...
function upvoteComment(url, request) {
  ...
  if (savedComment && user && database.users[user]) {
    if (savedComment.upvotedBy.indexOf(user) === -1) {
      savedComment.upvotedBy.push(user);

      if (savedComment.downvotedBy.indexOf(user) >= 0) {
        const downvotedByList = savedComment.downvotedBy;
        downvotedByList.splice(downvotedByList.indexOf(user), 1);
      }
...
function downvoteComment(url, request) {
    ...
    if (savedComment && user && database.users[user]) {
      if (savedComment.downvotedBy.indexOf(user) === -1) {
        savedComment.downvotedBy.push(user);

        if (savedComment.upvotedBy.indexOf(user) >= 0) {
          const upvotedByList = savedComment.upvotedBy;
          upvotedByList.splice(upvotedByList.indexOf(user), 1);
        }
